...
export async function generateStaticParams() {
  ...
  return articles
    .filter((article) => article.collection === "warehousing-and-fulfillment-resources")
...
export async function generateMetadata(props: ResourcePageProps): Promise<Metadata> {
  ...
  const article = await cmsClient.getResourceArticleBySlug(`warehousing-and-fulfillment-resources/${slug}`);
...
export default async function WarehousingResourceArticlePage(props: ResourcePageProps) {
  const { slug } = await props.params;
  const article = await cmsClient.getResourceArticleBySlug(`warehousing-and-fulfillment-resources/${slug}`);
